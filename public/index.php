...
<div class="flex-1 space-y-8">
    <?php if ($pinnedArticle): ?>
        <article class="border-b border-neutral-800 pb-6">
            <div class="text-sm text-neutral-400 mb-1">
                <?= date('d F Y H:i', strtotime($pinnedArticle[0]['publication_date'])); ?>
            </div>
            <h2 class="text-lg font-semibold">
                <?= htmlspecialchars($pinnedArticle[0]['title'], ENT_QUOTES); ?>
    ...
    <?php foreach ($articles as $article): ?>
        <article class="pb-6">
            <div class="text-sm text-neutral-400 mb-1">
                <?= date('d F Y H:i', strtotime($article['publication_date'])); ?>
            </div>
            <h2 class="text-lg font-semibold">
